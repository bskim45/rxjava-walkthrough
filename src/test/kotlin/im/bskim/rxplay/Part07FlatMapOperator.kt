 ...
 * RuleOfThumb 2: When you have Observable<Observable></Observable><T>> you probably need flatMap.
 *
 * @author sbalamaci
</T></T></T> */
class Part07FlatMapOperator : BaseTestObservables() {
    ...
    @Test
    fun flatMap() {
        val colors = Flowable.just("orange", "red", "green")
     ...
     */
    @Test
    fun flatMapSubstreamOperations() {
        val colors = Flowable.just("orange", "red", "green", "blue")

        val colorsCounted = colors
                .flatMap { colorName ->
                    val timer = Flowable.interval(2, TimeUnit.SECONDS)

                    simulateRemoteOperation(colorName) // <- Still a stream
                            .zipWith<Long, String>(timer, BiFunction{ v, _ -> v })
                            .count()
                            .map({ counter -> Pair(colorName, counter) })
                            .toFlowable()
                }
    ...
    /**
     * When you have a Stream of Streams - Observable<Observable></Observable><T>>
    </T> */
    @Test
    fun flatMapForProcessingAStreamOfStreams() {
        ...
        val countedColors = groupedColorsStream
                .flatMap { groupedFlow ->
                    groupedFlow
                            .count()
                            .map { countVal -> Pair(groupedFlow.key, countVal) }
                            .toFlowable()
                }
     ...
     */
    private fun simulateRemoteOperation(color: String): Flowable<String> {
        return Flowable.create({ subscriber ->
            for (i in 0 until color.length) {
